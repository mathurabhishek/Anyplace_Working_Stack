...
[[ "$#" != 1 ]] && echo "No cleaning has happened!" >> tiler_log.txt && exit 1  
#[[ "$#" != 1 ]] && echo "No cleaning has happened!" && echo "Press any key to continue" && read  && exit 1 abhishek
...
if [ ! -e $TILES_DIR ]; then
	echo "Creating tiles directory..." >> tiler_log.txt
	mkdir $TILES_DIR
fi
...
do
	if [[ (-f "$filename") && ("$filename" =~ $REGEX) ]]; then
		...
		echo "$filename - zoom: $zoom" >> tiler_log.txt

		# ensure that the directory for this zoom level exists
		zoomDir="$TILES_DIR/$zoom" 
		echo $zoomDir >> tiler_log.txt
		if [ ! -e $zoomDir ]; then
			echo "Creating directory for zoom: $zoom" >> tiler_log.txt
			mkdir $zoomDir
		fi
...
echo -e "Everything has been cleaned up!" >> tiler_log.txt
#echo "End of fix-tile-structure.sh .. Press any key to continue"
#read 